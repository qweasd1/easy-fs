...
class Directory extends Node {
  ...
  find(pattern) {
    let matches = this.findAll(pattern)
    if (matches.length === 0) {
      return null
    }
    else {
    ...
    for (let key in bundle) {
      ...
      if (typeof child === 'object') {
        ...
        if (child instanceof fs.ReadStream) {
          ...
          this._getOrCreateDir([key]).newBundle(child)
        }
      }
  ...
  _addChildToCache(name, child) {
    
    this._childrenCache[name] = child
  }
  ...
  _getOrCreateDir(paths) {
    ...
    if (dirName in this._childrenCache) {
      ...
      this._addChildToCache(dirName, dir)
    }
    if (paths.length > 0) {
